...
function assertMatchesNativeFunction(f) {
  ...
  assert.sameValue(/^function\b/.test(source), true);
  ...
  assert.sameValue(openCurly < closeCurly, true);

  var body = source.slice(openCurly + 1, closeCurly);
  assert.sameValue(/^\s*\[native code\]\s*$/.test(body), true);
  ...
  try {
    ...
    assert.sameValue(true, false, `${source} doesn't match NativeFunction`);
  }
}
