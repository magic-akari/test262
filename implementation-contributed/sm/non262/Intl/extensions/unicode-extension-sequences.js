...
for (const {locale, start, end} of testcases) {
    ...
    assert.compareArray(Intl.getCanonicalLocales(locale), [locale]);

    assert.sameValue(startOfUnicodeExtensions(locale), start);

    if (start >= 0)
        assert.sameValue(endOfUnicodeExtensions(locale, start), end);
}
