...
assert.sameValue(supported.length, 1);
assert.sameValue(supported[0], "az");

withLocale("az", () => {
    // Ensure the new default locale is now active.
    assert.sameValue(new Intl.Collator().resolvedOptions().locale, "az");

    // "az" is the active default locale, so explicitly requesting "az" should succeed.
    assert.sameValue(new Intl.Collator("az").resolvedOptions().locale, "az");

    // ICU doesn't provide a specialised "az-Cyrl" locale, so we fallback to "az".
    assert.sameValue(new Intl.Collator("az-Cyrl").resolvedOptions().locale, "az");

    // ICU doesn't provide a specialised "az-Cyrl-AZ" locale, so we fallback to "az".
    assert.sameValue(new Intl.Collator("az-Cyrl-AZ").resolvedOptions().locale, "az");
});
...
withLocale("az-Cyrl-AZ", () => {
    // Ensure the new default locale is now active.
    assert.sameValue(new Intl.Collator().resolvedOptions().locale, "az-Cyrl-AZ");
    ...
    assert.sameValue(new Intl.Collator("az-Cyrl").resolvedOptions().locale, "az-Cyrl");

    // "az-Cyrl-AZ" is the active default locale, so explicitly requesting "az-Cyrl-AZ"
    // should succeed.
    assert.sameValue(new Intl.Collator("az-Cyrl-AZ").resolvedOptions().locale, "az-Cyrl-AZ");
});
