...
for (let [timeZone, {timeZoneName, date, locales}] of Object.entries(tests)) {
  for (let [locale, expected] of Object.entries(locales)) {
    ...
    assert.sameValue(dtf.format(date), expected);

    let parts = dtf.formatToParts(date);
    assert.sameValue(parts.map(p => p.value).join(""), expected);
    assert.sameValue(parts.filter(p => p.type === "timeZoneName").length, 1);

    assert.sameValue(dtf.resolvedOptions().timeZoneName, timeZoneName);
  }
}
