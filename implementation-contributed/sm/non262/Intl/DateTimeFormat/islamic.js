...
function testIslamicTbla() {
    ...
    assert.sameValue(civilDate(monthYear, date), tabularDate(monthYear, date));

    // Day is different by one.
    var day = {day: "numeric"};
    assert.sameValue(Number(civilDate(day, date)) - Number(tabularDate(day, date)), -1);
}
...
function testIslamicRgsa() {
    ...
    assert.sameValue(sightingDate(dayMonthYear, date1), tabularDate(dayMonthYear, date1));
    assert.sameValue(sightingDate(dayMonthYear, date2), civilDate(dayMonthYear, date2));
}
...
function testIslamicUmalqura() {
    ...
    for (var [gregorian, ummAlQura] of dates) {
        ...
        assert.sameValue(parseInt(ummAlQuraDate(year, date), 10), ummAlQura.year);
        assert.sameValue(Number(ummAlQuraDate(month, date)), ummAlQura.month);
        assert.sameValue(Number(ummAlQuraDate(day, date)), ummAlQura.day);
    }
}
