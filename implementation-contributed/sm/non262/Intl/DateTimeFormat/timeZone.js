...
for (let timeZone of utcTimeZones) {
    for (let map of tzMapper) {
        ...
        assert.sameValue(dtf.resolvedOptions().timeZone, "UTC");
    }
}
...
for (let gmtOffset = -14; gmtOffset <= 12; ++gmtOffset) {
    ...
    for (let map of tzMapper) {
        let dtf = new Intl.DateTimeFormat(undefined, {timeZone: map(timeZone)});
        assert.sameValue(dtf.resolvedOptions().timeZone, timeZone);
    }
}
...
// |undefined| or absent "timeZone" option selects the default time zone.
{
    ...
    assert.sameValue(typeof tzAbsent, "string");
    assert.sameValue(typeof tzUndefined, "string");
    assert.sameValue(tzUndefined, tzAbsent);
    ...
    assert.sameValue(tzDefault, tzAbsent);
}
