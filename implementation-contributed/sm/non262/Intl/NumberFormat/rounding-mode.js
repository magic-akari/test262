...
for (let {value, options, roundingModes} of tests) {
  for (let [roundingMode, expected] of Object.entries(roundingModes)) {
    ...
    assert.sameValue(nf.format(value), expected, `value=${value}, roundingMode=${roundingMode}`);
    assert.sameValue(nf.resolvedOptions().roundingMode, roundingMode);
  }
}

// Default value is "halfExpand".
assert.sameValue(new Intl.NumberFormat().resolvedOptions().roundingMode, "halfExpand");
