...
for (let {value, options, roundingPriorities} of tests) {
  for (let [roundingPriority, expected] of Object.entries(roundingPriorities)) {
    ...
    assert.sameValue(nf.resolvedOptions().roundingPriority, roundingPriority);
    assert.sameValue(nf.format(value), expected, `value=${value}, roundingPriority=${roundingPriority}`);
  }
}

// Default value of "auto".
assert.sameValue(new Intl.NumberFormat().resolvedOptions().roundingPriority, "auto");
