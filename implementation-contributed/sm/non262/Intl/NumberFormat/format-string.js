...
for (let {value, expected} of tests) {
  for (let ws of strWhiteSpaceChar) {
    assert.sameValue(nf.format(ws + value), expected);
    assert.sameValue(nf.format(value + ws), expected);
    assert.sameValue(nf.format(ws + value + ws), expected);
  }
}
...
// Combine extreme values with other rounding modes.
{
  ...
  assert.sameValue(nf.format(".1e-999999998"), "0.00000000000000000000");
}
