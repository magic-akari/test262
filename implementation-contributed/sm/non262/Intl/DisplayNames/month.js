...
for (let [locale, localeTests] of Object.entries(tests)) {
  ...
  for (let [style, styleTests] of Object.entries(localeTests)) {
    ...
    assert.sameValue(resolved.type, "month");
    assert.sameValue(resolved.fallback, "code");

    for (let i = 0; i < 13; i++) {
      assert.sameValue(dn.of(i + 1), String(styleTests[i]));

      // Also works with strings.
      assert.sameValue(dn.of(String(i + 1)), String(styleTests[i]));

      // Also works with objects.
      assert.sameValue(dn.of(Object(i + 1)), String(styleTests[i]));
    }
  }
}
...
// Test fallback behaviour.
{
  ...
  assert.sameValue(dn2.resolvedOptions().calendar, "gregory");
  assert.sameValue(dn3.resolvedOptions().calendar, "gregory");

  // The Gregorian calendar doesn't have a thirteenth month.
  assert.sameValue(dn1.of("13"), "13");
  assert.sameValue(dn2.of("13"), "13");
  assert.sameValue(dn3.of("13"), undefined);
}
