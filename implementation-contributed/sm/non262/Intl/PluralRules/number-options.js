            ...
            for (let roundingIncrement of [undefined, 5000]) {
              for (let roundingMode of [undefined, "trunc"]) {
                for (let trailingZeroDisplay of [undefined, "stripIfInteger"]) {
                  ...
                  assert.sameValue(resolved.minimumIntegerDigits, minimumIntegerDigits ?? 1);
                  assert.sameValue(resolved.minimumFractionDigits, mnfd);
                  assert.sameValue(resolved.maximumFractionDigits, mxfd);
                  assert.sameValue(resolved.minimumSignificantDigits, mnsd);
                  assert.sameValue(resolved.maximumSignificantDigits, mxsd);
                  assert.sameValue(resolved.roundingPriority, roundingPriority ?? "auto");
                  assert.sameValue(resolved.roundingIncrement, roundingIncrement ?? 1);
                  assert.sameValue(resolved.roundingMode, roundingMode ?? "halfExpand");
                  assert.sameValue(resolved.trailingZeroDisplay, trailingZeroDisplay ?? "auto");
                }
              }
            }
          }
