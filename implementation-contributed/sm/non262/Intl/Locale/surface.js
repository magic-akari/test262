...
function assertProperty(object, name, desc) {
    assert.sameValue(desc === undefined || (typeof desc === "object" && desc !== null), true,
             "desc is a property descriptor");

    var actual = Object.getOwnPropertyDescriptor(object, name);
    if (desc === undefined) {
        assert.sameValue(actual, desc, `property ${String(name)} is absent`);
        return;
    }
    assert.sameValue(actual !== undefined, true, `property ${String(name)} is present`);

    var fields = ["value", "writable", "enumerable", "configurable", "get", "set"];
    for (var field of fields) {
        if (Object.prototype.hasOwnProperty.call(desc, field)) {
            assert.sameValue(actual[field], desc[field], `unexpected value for ${field}`);
        }
    }
}
