...
assert.sameValue(isSameCompartment(newRope, this), true);
...
function checkUtf8Equal(first, second) {
  ...
  assert.sameValue(first.length, firstRead);
  assert.sameValue(second.length, secondRead);

  assert.sameValue(firstWritten, secondWritten);

  for (var i = 0; i < firstWritten; ++i) {
    assert.sameValue(firstBuffer[i], secondBuffer[i]);
  }
}
