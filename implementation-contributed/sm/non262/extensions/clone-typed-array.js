...
function assertArraysEqual(a, b) {
    assert.sameValue(a.constructor, b.constructor);
    assert.sameValue(a.length, b.length);
    for (var i = 0; i < a.length; i++)
        assert.sameValue(a[i], b[i]);
}
...
function test() {
    ...
    assert.sameValue(b[0].buffer, b[1].buffer);
    assert.sameValue(b[1].byteOffset, 1);
    assert.sameValue(b[1].byteLength, 3);
    assert.sameValue(b[1].buffer.byteLength, 4);
    ...
    assert.sameValue("prop" in a[0].buffer, false);
    assert.sameValue("prop" in a[1].buffer, false);
    assert.sameValue("loop" in a[0].buffer, false);
    assert.sameValue("loop" in a[1].buffer, false);
    assert.sameValue("loops" in a[0].buffer, false);
    assert.sameValue("loops" in a[1].buffer, false);
}
