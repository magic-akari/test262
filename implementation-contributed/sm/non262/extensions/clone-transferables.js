...
function test() {
    for (var {scope, size} of buffer_options()) {
        ...
        assert.sameValue(old.byteLength, 0);
        assert.sameValue(copy[0] === copy[1], true);
        copy = copy[0];
        assert.sameValue(copy.byteLength, size);
        ...
        for (var ctor of constructors) {
            ...
            assert.sameValue(buf.byteLength, size);
            assert.sameValue(buf, old_arr.buffer);
            if (!dataview)
                assert.sameValue(old_arr.length, size / old_arr.BYTES_PER_ELEMENT);
            ...
            if (!dataview) {
                assert.sameValue(old_arr.length, 0,
                         "donor typed array should be detached");
            }
            assert.sameValue(copy_arr.buffer.byteLength == size, true);
            if (!dataview)
                assert.sameValue(copy_arr.length, size / old_arr.BYTES_PER_ELEMENT);
        ...
        for (var ctor of constructors) {
            ...
            assert.sameValue(buf.byteLength, 0,
                     "donor array buffer should be detached");
            if (!dataview) {
                assert.sameValue(old_arr.byteLength, 0,
                         "donor typed array should be detached");
                assert.sameValue(old_arr.length, 0,
                         "donor typed array should be detached");
            }
        ...
        if (size >= 4) {
            ...
            assert.sameValue(view.length, 0); // Underlying buffer now detached.
            assert.sameValue(viewCopy[0], 2);
        }
