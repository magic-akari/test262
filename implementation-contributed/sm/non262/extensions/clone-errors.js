                                       ...
                                       ['SameProcess', 'DifferentProcess']])
{
  ...
  assert.sameValue(caught, true, `${write_scope} clone buffer should not be deserializable as ${read_scope}`);
}
...
if (shouldThrow) {
  assert.sameValue(exc.message.includes("bad serialized structured data"), true);
  assert.sameValue(exc.message.includes("extra data"), true);
}
...
function testInnerExtraData() {
  ...
  if (shouldThrow) {
    assert.sameValue(Boolean(exc), true);
    assert.sameValue(exc.message.includes("extra data"), true);
    print(`PASS with FUZZING: Found expected exception "${exc.message}"`);
  } else {
    assert.sameValue(new BigUint64Array(d.CBA)[0].toString(16), "1cedc0ffee");
    assert.sameValue(d.ABC, 13);
