...
function compareProperties(a, b, stack, path) {
    ...
    for (var i = 0; i < pa.length; i++) {
        var propname = pa[i][0];
        assert.sameValue(typeof propname, "string", "clone should not have E4X properties " + path);
    ...
    assert.sameValue(aNames.join(","), bNames.join(","), path);

    // Check that the lists are the same when including array indexes.
    function byName(a, b) { a = a[0]; b = b[0]; return a < b ? -1 : a === b ? 0 : 1; }
    pa.sort(byName);
    pb.sort(byName);
    assert.sameValue(pa.length, pb.length, "should see the same number of properties " + path);
    for (var i = 0; i < pa.length; i++) {
        var aName = pa[i][0];
        var bName = pb[i][0];
        assert.sameValue(aName, bName, path);

        var path2 = isIndex(aName) ? path + "[" + aName + "]" : path + "." + aName;
        var da = pa[i][1];
        var db = pb[i][1];
        assert.sameValue(da.configurable, shouldBeConfigurable(ca, aName), path2);
        assert.sameValue(da.writable, shouldBeWritable(ca, aName), path2);
        assert.sameValue("value" in da, true, path2);
        var va = da.value;
        var vb = b[pb[i][0]];
...
function isClone(a, b) {
    ...
    while (stack.length > 0) {
        ...
        if (typeof x !== "object" || x === null) {
            // x is primitive.
            assert.sameValue(x, y, "equal primitives");
        } else if (x instanceof Date) {
            assert.sameValue(x.getTime(), y.getTime(), "equal times for cloned Dates");
        } else if (memory.has(x)) {
            // x is an object we have seen before in a.
            assert.sameValue(y, memory.get(x), "repeated object the same");
            assert.sameValue(rmemory.get(y), x, "repeated object's clone already seen");
        } else {
            // x is an object we have not seen before.
	    // Check that we have not seen y before either.
            assert.sameValue(rmemory.has(y), false);

            var xcls = classOf(x);
            var ycls = classOf(y);
            assert.sameValue(xcls, ycls, "same [[Class]]");

            // clone objects should have the default prototype of the class
            assert.sameValue(Object.getPrototypeOf(x), this[xcls].prototype);
...
function check(val) {
    ...
    assert.sameValue(isClone(val, clone), true);
    return clone;
}
...
var clone = check(ar);
assert.sameValue(clone[0] === clone[1], false);
...
b = [ v, v, { 'date': v } ];
clone = check(b);
assert.sameValue(clone[0] === clone[1], true);
assert.sameValue(clone[0], clone[2]['date']);
assert.sameValue(clone[0] === v, false);
