...
assert.sameValue("defineProperty" in Object, true);
assert.sameValue(Object.defineProperty.length, 3);
...
function assert.sameValue(v1, v2, m)
{
  if (!SameValue(v1, v2))
  {
...
function compareDescriptors(d1, d2)
{
  if (d1 === undefined)
  {
    assert.sameValue(d2, undefined, "non-descriptors");
    return;
  }
  if (d2 === undefined)
  {
    assert.sameValue(true, false, "descriptor-equality mismatch: " + uneval(d1) + ", " + uneval(d2));
    return;
  }
  ...
  {
    var p = props[i];
    assert.sameValue(p in d1, p in d2, p + " different in d1/d2");
    if (p in d1)
      assert.sameValue(d1[p], d2[p], p);
  }
}
...
function examine(desc, field, allowDefault)
{
  if (field in desc)
    return desc[field];
  assert.sameValue(allowDefault, true, "reimplementation error");
  switch (field)
  {
    case "value":
    case "get":
    ...
    case "configurable":
      return false;
    default:
      assert.sameValue(true, false, "bad field name: " + field);
  }
}
...
CustomObject.prototype =
{
  ...
  {
    assert.sameValue(typeof propname, "string", "non-string propname");
    ...
    else if (IsDataDescriptor(current) && IsDataDescriptor(desc))
    {
      ...
      {
        assert.sameValue(examine(current, "configurable"), true,
                 "spec bug step 10(b)");
      }
    }
    // Step 11.
    ...
    {
      assert.sameValue(IsAccessorDescriptor(current) && IsAccessorDescriptor(desc),
               true,
               "spec bug");
...
var ReimplTest =
  {
    ...
    defineProperty: function defineProperty(obj, propname, propdesc)
    {
      assert.sameValue(obj instanceof CustomObject, true, "obj not instanceof CustomObject");
