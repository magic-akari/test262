...
assert.sameValue(results.length, 5);
assert.sameValue(results[0], 'then result');
assert.sameValue(results[1], 'catch rejection');
assert.sameValue(results[2], 'chained then with val: first then rval');
assert.sameValue(results[3], 'then after catch with val: 2');
assert.sameValue(results[4], 'p1 then, then,p2 catch, then');
