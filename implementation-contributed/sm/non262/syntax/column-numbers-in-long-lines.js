...
- Reflect
info: |
  'uses Reflect.parse(..., { loc: true}) to trigger the column-computing API'
description: |
  pending
esid: pending
---*//*
...
function checkLoc(node, expectedStart, expectedEnd)
{
  ...
  assert.sameValue(start.line, expectedStart[0],
           "start line number must be as expected");
  assert.sameValue(start.column, expectedStart[1],
           "start column number must be as expected");

  let end = node.loc.end;

  assert.sameValue(end.line, expectedEnd[0], "end line number must be as expected");
  assert.sameValue(end.column, expectedEnd[1],
...
const columnsAreCodePoints = (function()
{
  ...
  assert.sameValue(script.type, "Program");
  assert.sameValue(script.loc.start.line, 1);
  assert.sameValue(script.loc.end.line, 1);
  assert.sameValue(script.loc.start.column, 1);
  checkColumn(script.loc.end.column, 8, 12);

  var body = script.body;
  assert.sameValue(body.length, 1);

  var stmt = body[0];
  assert.sameValue(stmt.type, "ExpressionStatement");
  assert.sameValue(stmt.loc.start.line, 1);
  assert.sameValue(stmt.loc.end.line, 1);
  assert.sameValue(stmt.loc.start.column, 1);
  checkColumn(stmt.loc.end.column, 8, 12);

  var expr = stmt.expression;
  assert.sameValue(expr.type, "Literal");
  assert.sameValue(expr.value, "😱😱😱😱");
  assert.sameValue(expr.loc.start.line, 1);
  assert.sameValue(expr.loc.end.line, 1);
  assert.sameValue(expr.loc.start.column, 1);
  checkColumn(expr.loc.end.column, 7, 11);

  var checkResult = columnTypes.join(",");

  assert.sameValue(checkResult === "p,p,p" || checkResult === "u,u,u", true,
...
function testSimple()
{
  ...
  assert.sameValue(lengthInCodePoints(simpleCode), CodePointLength,
  ...
  assert.sameValue(OverallCodeUnitCount > CodePointLength, true,
  ...
  assert.sameValue(OverallCodeUnitCount, 450,
           "code unit count computation produces this value");

  // The overall computed number of code units matches the string length.
  assert.sameValue(simpleCode.length, OverallCodeUnitCount, "string length must match");
  ...
  for (let i = 0; i < NumArrayElements; i++)
  {
    ...
    assert.sameValue(arrayElems.length, 1);

    let str = arrayElems[0];
    assert.sameValue(str.type, "Literal");
    assert.sameValue(str.value,
...
function testGuaranteedSingleUnit()
{
  ...
  assert.sameValue(mixedChunksCode.length, 15);
  assert.sameValue(lengthInCodePoints(mixedChunksCode), 15);

  mixedChunksCode +=
    " ".repeat(128 - mixedChunksCode.length); // column 16, offset 15
  assert.sameValue(mixedChunksCode.length, 128);
  assert.sameValue(lengthInCodePoints(mixedChunksCode), 128);
  ...
  assert.sameValue(mixedChunksCode.length, 256);
  assert.sameValue(lengthInCodePoints(mixedChunksCode), 256);
  ...
  assert.sameValue(mixedChunksCode.length, 384 + 1);
  assert.sameValue(lengthInCodePoints(mixedChunksCode), 324);
  ...
  assert.sameValue(mixedChunksCode.length, 396);
  assert.sameValue(lengthInCodePoints(mixedChunksCode), 335);
...

  {
    ...
    assert.sameValue(bigArrayOfEmpties.type, "ArrayExpression");
    assert.sameValue(bigArrayOfEmpties.elements.length, 42 + 1);
    bigArrayOfEmpties.elements.forEach((elem, i) => {
      assert.sameValue(elem.type, "ArrayExpression");
      assert.sameValue(elem.elements.length, 0);
      if (i !== 42)
    ...
    assert.sameValue(multiUnitStringLiteral.type, "Literal");
    assert.sameValue(multiUnitStringLiteral.value, "😱".repeat(61));
...

  {
    ...
    assert.sameValue(stringLiteralWithEmbeddedTerminator.type, "Literal");
    assert.sameValue(stringLiteralWithEmbeddedTerminator.value, "\u2028");
