...
for (let TA of TAConstructors) {
  ...
  for (let size of sizes) {
    ...
    assert.sameValue(ta[size - 1] === sorted[size - 1], false);
    ...
    assert.sameValue(Atomics.wait(controller, 0, 0), "ok");
    ...
    for (let i = 0; i < size; ++i) {
      assert.sameValue(ta[i], zero, `${TA.name} at index ${i} for size ${size}`);
    }
  }
}
